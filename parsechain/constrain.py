...
class Constrain:
    ...
    def format_prompt(self, grammars, placeholders=None, examples=None):
        ...
        if not grammars:
            raise ValueError(
                'You need to provide a list of grammars to format the prompt!')
        ...
        if isinstance(self.prompt, Prompt):
            ...
            if placeholders: self.initial_prompt += " ".join(list(placeholders.values()))
    ...
    def parse_helper(self, return_value):
        ...
        if isinstance(return_value, str):
            return_value = return_value.replace('```json', '```').replace('```xml', '```').replace('```toml', '```') # Sometimes, name is added next to the terminals
            returned_objects = [split_string.replace('\n', '') for split_string in return_value.split('```')[1::2] if split_string.replace('\n', '').strip()] # Splitting between terminals and removing empty strings
        else:
